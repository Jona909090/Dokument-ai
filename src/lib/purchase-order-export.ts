...
function partyText(party: PurchaseOrderParty) {
  return [
    party.name,
    party.address,
    party.cityPostalCode,
    party.taxNumber && `OIB / PIB: ${party.taxNumber}`,
    party.contactPerson && `Kontakt: ${party.contactPerson}`,
    [party.phone, party.email].filter(Boolean).join(" · "),
  ]
    .filter(Boolean)
    .join("\n");
}
function metadata(data: PurchaseOrderData) {
  return [
    ["Željena isporuka", formatDocumentDate(data.desiredDeliveryDate, "hr")],
    ["Mjesto isporuke", data.deliveryPlace],
    ["Način isporuke", data.deliveryMethod],
    ["Rok plaćanja", data.paymentDeadline],
    ["Način plaćanja", data.paymentMethod],
    ["Referenca ponude", data.offerReference],
    ["Projekt / gradilište", data.project],
...
export async function downloadPurchaseOrderPdf(document: GeneratedDocument) {
  ...
  const header = data.showPrices
    ...
    : ["R.br.", "Naziv / opis", "Kol.", "JM"];
  const widths = data.showPrices
    ? [25, "*", 34, 28, 58, 38, 30, 65]
    : [30, "*", 55, 45];
  const rows: Content[][] = data.items
    .filter((item) => item.isVisible !== false)
    .map((item, index) => {
    const amount = calculatePurchaseOrderItem(item);
    const description: Content = item.description
      ? {
          stack: [
    ...
    return data.showPrices
      ? [
          ...
          `${item.discountRate}%`,
          `${item.taxRate}%`,
          money(amount.total, data.currency),
        ]
      : base;
    });
  const content: Content[] = [
    {
      columns: [
        {
          stack: document.images?.logo
            ? [
                { image: document.images.logo, fit: [95, 38] },
                { text: partyText(data.buyer), margin: [0, 4, 0, 0] },
              ]
            : [{ text: partyText(data.buyer), bold: true }],
        },
        {
          stack: [
            {
            ...
            {
              text: `Br. ${data.orderNumber || "—"}`,
              bold: true,
              alignment: "right",
              margin: [0, 4, 0, 0],
            },
            {
              text: `${formatDocumentDate(data.issueDate, "hr")}${data.issuePlace ? ` · ${data.issuePlace}` : ""}`,
              alignment: "right",
            },
          ],
        },
      ...
      columns: [
        {
          stack: [
            ...
            { text: partyText(data.supplier) },
          ],
        },
        {
          stack: [
            {
              text: "ISPORUKA I PLAĆANJE",
              bold: true,
              fontSize: 8,
              margin: [0, 0, 0, 4],
            },
            ...metadata(data).map(([label, value]) => ({
  ...
  if (data.showPrices)
    content.push({
      table: {
        widths: [110, 90],
        body: [
          ["Ukupno bez PDV-a", money(totals.gross, data.currency)],
          ...
          [
            ...
            { text: money(totals.total, data.currency), bold: true },
          ],
        ],
      },
  ...
  for (const [title, value] of [
    ["NAPOMENA", data.note],
    ["UVJETI ISPORUKE", data.deliveryTerms],
    ["UVJETI PLAĆANJA", data.paymentTerms],
  ])
    if (value)
      content.push({
        stack: [
          { text: title, bold: true, fontSize: 8 },
          { text: value, margin: [0, 3, 0, 0] },
        ],
        margin: [0, 10, 0, 0],
        unbreakable: true,
      });
  const pdfSignatureCells: Content[] = [
    ...
    return {
      stack: [
        { text: title as string, bold: true, alignment: "center" },
        { text: entry.name, alignment: "center", margin: [0, 20, 0, 0] },
        { text: entry.role, alignment: "center", color: "#64748b" },
        { text: formatDocumentDate(entry.date, "hr"), alignment: "center" },
      ],
    };
  });
  content.push({
  ...
  if (approvalImages.length)
    content.push({
      columns: [{ text: "" }, ...approvalImages],
      columnGap: 10,
      margin: [0, 8, 0, 0],
      unbreakable: true,
    });
  const definition: TDocumentDefinitions = {
    ...
    footer: (page, pages) => ({
      columns: [
        {
          text: data.buyer.name || "Narudžbenica",
          color: "#64748b",
          fontSize: 7,
        },
        {
    ...
    info: {
      title: `Narudžbenica ${data.orderNumber}`,
      creator: data.buyer.name || "Dokument AI",
    },
  };
  configured
...
function dataUrl(dataUrl: string) {
...
export async function downloadPurchaseOrderDocx(document: GeneratedDocument) {
  ...
  const headerLabels = data.showPrices
    ? [
        "R.br.",
        "Naziv / opis",
        "Kol.",
  ...
  const tableRows = [
    ...
    ...data.items.filter((item) => item.isVisible !== false).map((item, index) => {
      const amount = calculatePurchaseOrderItem(item);
      const values = [
        String(index + 1),
      ...
      new TableRow({
        children: [
          new TableCell({
            ...
            children: [p(partyText(data.buyer), true)],
          }),
          new TableCell({
            children: [
              new Paragraph({
              ...
              new Paragraph({
                ...
                children: [
                  new TextRun({
                    text: `Br. ${data.orderNumber}\n${formatDocumentDate(data.issueDate, "hr")} ${data.issuePlace}`,
                    font: "Arial",
                    size: 18,
                  }),
                ],
  ...
  const partyTable = new Table({
    ...
    rows: [
      new TableRow({
        children: [
          new TableCell({
            children: [p("DOBAVLJAČ", true), p(partyText(data.supplier))],
          }),
          new TableCell({
            children: [
              p("ISPORUKA I PLAĆANJE", true),
              ...metadata(data).map(([label, value]) =>
                p(`${label}: ${value}`),
              ),
            ],
  ...
  if (data.showPrices)
    children.push(
      p(
        `Ukupno bez PDV-a: ${money(totals.gross, data.currency)}\nPopust: ${money(totals.discount, data.currency)}\nPorezna osnovica: ${money(totals.subtotal, data.currency)}\nPDV: ${money(totals.tax, data.currency)}\nUKUPNO S PDV-om: ${money(totals.total, data.currency)}`,
        true,
      ),
    );
  for (const [title, value] of [
    ...
    if (value) children.push(p(`${title}\n${value}`, true));
    ...
    return new TableCell({
      children: [
        new Paragraph({
          ...
          children: [
            new TextRun({
              text: `${title}\n\n${entry.name}\n${entry.role}\n${formatDocumentDate(entry.date, "hr")}`,
              bold: true,
              font: "Arial",
              size: 16,
            }),
        ...
        children: [
          ...
          document.images.stamp &&
            new ImageRun({
              ...
              transformation: { width: 65, height: 48 },
          ...
          default: new Footer({
            children: [
              new Paragraph({
                alignment: AlignmentType.RIGHT,
                children: [
                  new TextRun({ text: "Stranica ", font: "Arial", size: 15 }),
                  new TextRun({
