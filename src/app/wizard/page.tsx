...
export default async function WizardPage({ searchParams }: PageProps<"/wizard">) { const params = await searchParams; const typeValue = typeof params.type === "string" ? params.type : undefined; const prompt = typeof params.prompt === "string" ? params.prompt : undefined; if (typeValue === "purchase-order") return <DocumentGenerator initialType="purchase-order" originalPrompt={prompt} />; return <SmartWizard initialType={isDocumentType(typeValue) ? typeValue : null} initialPrompt={prompt} />; }
